...
trait Solver extends GameDef {
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): Stream[(Block, List[Move])] =
    for((neighbor, move) <- b.legalNeighbors.toStream) yield (neighbor, move :: history)
  ...
  def from(initial: Stream[(Block, List[Move])], explored: Set[Block]): Stream[(Block, List[Move])] =
    if(initial.isEmpty) Stream.empty
      ...
      val (block, moves): (Block, List[Move])       = initial.head
      val neighbors: Stream[(Block, List[Move])]    = neighborsWithHistory(block, moves)
      val newNeighbors: Stream[(Block, List[Move])] = newNeighborsOnly(neighbors, explored)

      initial ++ from(initial.tail ++ newNeighbors, explored + block)
    }
